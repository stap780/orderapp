...
$('#invoice_check_button').click(function() {
	$('#my_invoiceout_check').toggle();
});
  
$('#vozvrat_check_button').click(function() {
	$('#my_vozvrat_check').toggle();
});
...
$('#q_company_title').click(function(){
	alert('ki');
});


	
$('#purchase_list_q_submit').click(function(){
alert('ki');
	if ($('#q_company_id_eq').val() != ''){
		var companyId = $('#q_company_id_eq').val();
		$.ajax({
		type: "GET",
		dataType: "json",
		//data: {id: companyId},
		...
		success: function(data){
		$('#q_company_title').val(data.title);
		}
	  });
